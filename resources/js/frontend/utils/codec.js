/**
 * Compact MD5 (UTF-8). Returns 32-char lowercase hex by default.
 */
...
/**
 * @param {string} str
 * @param {{ upper?: boolean, short16?: boolean }} [opts]
 */
export function md5(str, opts = {}) {
  ...
  let hex = rhex(state[0]) + rhex(state[1]) + rhex(state[2]) + rhex(state[3]);
  if (opts.short16) {
    hex = hex.slice(8, 24);
  }
...
export function urlEncode(str, mode = 'component') {
  const s = String(str ?? '');
