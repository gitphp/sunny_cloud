...
return new class extends Migration
{
    ...
    public function up(): void
    {
        Schema::create('user_account', function (Blueprint $table) {
            $table->id()->comment('用户唯一主键ID');
            $table->string('user_name', 32)->default('')->comment('账号用户名，唯一，可用于登录');
            $table->string('nick_name', 32)->comment('用户昵称');
            ...
            $table->string('lock_reason', 255)->default('')->comment('冻结/封禁原因');
        ...
        if (Schema::getConnection()->getDriverName() === 'mysql') {
            DB::statement('ALTER TABLE `user_account` AUTO_INCREMENT = 934035802554576899');
        }
    }
